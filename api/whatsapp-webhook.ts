...
export default async function handler(req: Request) {
  ...
  if (req.method === 'GET') {
    return new Response(JSON.stringify({ status: 'ok', message: 'Webhook is running ready for Chatfuel' }), { 
      status: 200,
  ...
  if (req.method === 'POST') {
    try {
      ...
      // Extraer datos según formato de Chatfuel: { "messages": [{"text": "...", "sender": {"id": "..."}}] }
      let contactPhone = '';
      let messageText = '';
      ...
      if (body.messages && body.messages.length > 0) {
        ...
        // Fallback robusto por si Chatfuel envía el texto en su formato nativo JSON API
